        ...
        while i < len(lines):
            ...
            if line[:3] == "###":
                ...
                while j < len(lines) and lines[j][0] == "|":
                    ...
                    mem = (
                        els[mem_id] if els[mem_id] != "-" and els[mem_id] != "" else -1
                    )
                    ...
                    if ms_id and els[ms_id] != "-" and els[ms_id] != "":
                        model["Results"]["Metrics"]["mIoU(ms+flip)"] = float(els[ms_id])
                    models.append(model)
                    j += 1
                i = j
            else:
...
def update_model_index():
    ...
    model_index = {
        "Import": [osp.relpath(yml_file, MMSEG_ROOT) for yml_file in yml_files]
    }
