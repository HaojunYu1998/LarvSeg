...
def img_recall(hist, topk_label, mask=None):
    ...
    recall = torch.gather(gt_label, 1, topk_label).sum(dim=1) / (hist > 0).float().sum(
        dim=1
    )
    if (hist > 0).sum() == 0:
...
def img_accuracy(hist, topk_label, mask=None):
    ...
    if mask is not None:
        gt_label[torch.logical_not(mask)] = 0
        recall = torch.gather(gt_label, 1, topk_label).sum(dim=1) / torch.gather(
            mask, 1, topk_label.repeat(mask.size(0), 1)
        ).sum(dim=1)
    else:
...
def get_average_precision(hist, topk_label):
    gt_label = hist > 0
    k_index = topk_label.new_tensor(
        [[i for i in range(topk_label.shape[1])]] * topk_label.shape[0]
    ).float()  # B k
    inverted_label_index = float("inf") * topk_label.new_ones(
        (hist.shape[0], hist.shape[1])
    )
    inverted_label_index.scatter_(
        1, topk_label, k_index
    )  # B o, 'inf' for unpredicted classes, prediction rank for predicted classes
    ap_list = hist.new_zeros(
        hist.shape[0], dtype=torch.float
    )  # calculate AP for each image in the batch
    for i in range(hist.shape[0]):
        V, _ = inverted_label_index[
            i, gt_label[i]
        ].sort()  # for every groundtruth class, get its prediction rank (could be 'inf')
