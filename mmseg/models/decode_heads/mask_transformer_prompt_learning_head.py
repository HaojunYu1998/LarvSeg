...
from mmseg.models.clip_adapter import build_prompt_learner, ClipAdapter
...
class MaskTransformerPromptLearningHead(BaseDecodeHead):
    def __init__(
        ...
        **kwargs,
        ...
        self.clip_adapter = ClipAdapter(clip_model_name, cls_prompt_learner)
    ...
    def _sample_imagenet(self, seg_mask, seg_embed, cam_label, img_labels):
        ...
        if self.imagenet_pseudo_label:
            prior_bucket = [
                cam.topk(int((cam > 0).sum())).indices + b * H * W
            ...
            for b, cam in enumerate(cam_label):
                ...
                if self.use_pairwise_affinity and (cam_score > self.cam_thresh):
                    ...
                    pa_inds = (
                        (cos_sim > self.pairwise_affinity_thresh).nonzero().flatten()
                    )
