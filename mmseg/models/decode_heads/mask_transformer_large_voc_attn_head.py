...
class MaskTransformerLargeVocAttnHead(BaseDecodeHead):
    ...
    def _attn_head(self, x, y, scores, masks):
        ...
        for mask, mask2, label in zip(masks, masks2, labels):
            ...
            for l in range(self.cls_num):
                m = mask[:, l]
                if l in label:
                    m = self.alpha * mask[:, l] + (1 - self.alpha) * mask2[:, label.index(l)]
                mask2_list.append(m)
    ...
    def losses(self, seg_mask, seg_mask2, seg_embed, seg_score, seg_label):
        ...
        # classification task
        if self.weakly_supervised:
            seed_mask, seed_label = self._weak_sample(seg_mask, seg_mask2, seg_embed, seg_score, seg_label)
        else:
            seed_mask, seed_label = self._sample(seg_mask, seg_mask2, seg_embed, seg_label)
        # no valid label
