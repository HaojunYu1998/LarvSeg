...
class ImageNet21K(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        while not osp.exists(osp.join(self.ann_dir, ann_info["seg_map"])):
            idx_ = random.choice([i for i in range(len(self.img_infos))])
    ...
    def prepare_train_img(self, idx):
        ...
        try:
            ...
            results["gt_semantic_seg"] = DC(
                torch.zeros_like(results["gt_semantic_seg"].data) + _class, stack=True
            )
        except:
            for _ in range(1000):
                try:
...
class ImageNet11K(ImageNet21K):
    ...
    def prepare_train_img(self, idx):
        try:
            ...
            results["gt_semantic_seg"] = DC(
                torch.zeros_like(results["img"].data[[0]]).long() + _class, stack=True
            )
        except:
            for _ in range(1000):
                try:
    ...
    def prepare_test_img(self, idx):
        ...
        results["gt_semantic_seg"] = [
            DC(
                torch.zeros_like(results["img"][i].data[[0]]).long() + _class,
                stack=True,
            )
